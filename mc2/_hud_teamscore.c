...
onPlayerConnect()
{
	for(;;)
	{
		...
		player.hud_alliedscore.y = 26-8;
		player.hud_alliedscore.font = "default";
		player.hud_alliedscore.fontscale = 2-0.7;
		...
		player.hud_axisscore.y = 48-16;
		player.hud_axisscore.font = "default";
		player.hud_axisscore.fontscale = 2-0.7;
	    ...
	    player.hud_alliedscorelimit.y = 26-8;
	    player.hud_alliedscorelimit.font = "default";
	    player.hud_alliedscorelimit.fontscale = 2-0.7;
	    ...
	    player.hud_axisscorelimit.y = 48-16;
	    player.hud_axisscorelimit.font = "default";
	    player.hud_axisscorelimit.fontscale = 2-0.7;
	    player.hud_axisscorelimit.archived = false;
	    player.hud_axisscorelimit.label = (&"MP_SLASH");

		player.hud_alliedicon setShader(game["hudicon_allies"], 24-8, 24-8);
		player.hud_axisicon setShader(game["hudicon_axis"], 24-8, 24-8);
...
updatePlayerHUD()
{
	...
	if(winningteam == "allies")
	{
		if(self.hud_alliedicon.y != 28)
			self.hud_alliedicon.y = 28-10;
		if(self.hud_alliedscore.y != 26)
			self.hud_alliedscore.y = 26-8;
	    if(self.hud_alliedscorelimit.y != 26)
	    	self.hud_alliedscorelimit.y = 26-8;
		if(self.hud_axisicon.y != 50)
			self.hud_axisicon.y = 50-18;
		if(self.hud_axisscore.y != 48)
			self.hud_axisscore.y = 48-16;
	    if(self.hud_axisscorelimit.y != 48)
	    	self.hud_axisscorelimit.y = 48-16;
	}
	else if(winningteam == "axis")
	{
		if(self.hud_axisicon.y != 28)
			self.hud_axisicon.y = 28-10;
		if(self.hud_axisscore.y != 26)
			self.hud_axisscore.y = 26-8;
	    if(self.hud_axisscorelimit.y != 26)
	    	self.hud_axisscorelimit.y = 26-8;
		if(self.hud_alliedicon.y != 50)
			self.hud_alliedicon.y = 50-18;
		if(self.hud_alliedscore.y != 48)
			self.hud_alliedscore.y = 48-16;
	    if(self.hud_alliedscorelimit.y != 48)
	    	self.hud_alliedscorelimit.y = 48-16;
	}
