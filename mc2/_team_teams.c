...
#include mc2\_utils_models;

init()
{
	level.mc2_allowteamselect = cvardef("mc2_allow_team_select", 1, 0, 1, "int");
...
//	self.pers["savedmodel"] = maps\mp\_utility::saveModel();
	self.pers["savedmodel"] = saveModel(); //from _utils_models.gsc
}
...
//Test Client Code Split off into own file
