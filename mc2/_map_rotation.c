...
init()
{
	level.fixmaprotation 		= cvardef( "scr_mc2_fix_maprotation", 0, 0, 1, "int" );	
	level.randommaprotation 	= cvardef( "scr_mc2_random_maprotation", 0, 0, 2, "int" );	
	level.rotateifempty 		= cvardef( "scr_mc2_rotate_if_empty", 0, 0, 1800, "int" );
	level.fallbackgametypemap 	= cvardef( "scr_mc2_fallback_gametype_map", "tdm mp_brecourt", "", "", "string" );
...
{	
	...
	for( i = 0; i < maps.size; i++ )
	{
		...
		if( ( newmaprotation.size + temp.size ) > 975 )
		{
			iprintlnbold( &"mc2_FIXMAPROT_LIMITING" );
			break;
		}
		newmaprotation += temp;
...
GetMapRotation( random, current, number )
{
	...
	for( i=0; i < temparr.size; )
	{
		switch( temparr[i] )
		{
			...
			default:
				iprintlnbold( "mc2 mod: Error in Map Rotation" );
