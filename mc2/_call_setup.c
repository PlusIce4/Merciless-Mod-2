...
*/
	...
	logprint ("ERROR : incompatible gametype " + getcvar ("g_gametype") + " and map " + getcvar ("mapname") + "\n");
	
	if (getcvar ("sv_maprotationcurrent") == "")
	{
		array = strtok (level.awe_fallbackgametypemap, " ");
		fallback_gametype = array[0];
		fallback_map = array[1];
		logprint ("Falling back to gametype " + fallback_gametype + " and map " + fallback_map + "\n");
		setcvar ("sv_maprotationcurrent", " gametype " + fallback_gametype + " map " + fallback_map);
	}
	else
		logprint ("Rotating\n");
