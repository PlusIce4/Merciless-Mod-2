...
init()
{
	level.awe_allowteamselect = cvardef("awe_allow_team_select", 1, 0, 1, "int");
	...
	if(getcvar("g_gametype") != "dm" && getcvar("g_gametype") )
	{
...
balanceTeams()
{
	...
	{	
		if(AlliedPlayers.size > (AxisPlayers.size + 1))
		{
			...
			if ((getcvar ("g_gametype") == "sd") || (getcvar ("g_gametype") == "esd"))
...
{
	if ((! level.awe_allowteamselect) && (! isdefined (self.awe_clan_switch_team)))
		return false;
...
updateTeamChangeCvars()
{
	...
	if(!level.awe_allowteamselect)
	{
		joinallies = 2;
		joinaxis = 2;
	}
	else if(getcvar("g_gametype") == "dm" || level.maxclients < 2)
	{
		joinallies = 1;
		joinaxis = 1;
	}
...
setPlayerModels()
{
	// Default soldier types if none are specified in the map script
	switch (game["allies"])
	{
		case "american" :
