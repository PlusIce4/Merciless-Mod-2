...

{
	menuDef
	{
		...
		onEsc 
		{
			close weapon_british_sniper;
		}
		onOpen 
		{
			hide enfieldscoped_info;
...
// MENU CHOICES
		...
		execKey "1" { play "mouse_click"; scriptMenuResponse "springfield_mp"; }
		execKey "2" { play "mouse_click"; scriptMenuResponse "scoped_garand_mp"; }
	...
	itemDef 
		{
			name			"button_scopedgarand"
			visible			1
			rect			16 66 60 60
			origin			ORIGIN_MENUWINDOW
			forecolor		1 1 1 1
			type			ITEM_TYPE_BUTTON
			text			"@MC2_2_SCOPEDGARAND"
			textfont		UI_FONT_NORMAL
			textscale		.24
			textalignx		0
			textaligny		16
			dvartest		"ui_allow_scoped_garand"
			showDvar		{ "1" }
			action
			{
				play "mouse_click";
				scriptMenuResponse "scoped_garand_mp";
			}
			onFocus
			{
				hide springfield_info;
				play "mouse_over";
				show scopedgarand_info;
				show weapon_propertiestext;
				show grenade_info;
			}
		}
...
// WEAPON IMAGES
		...
		itemDef
				{
				name			"scopedgarand_info"
				visible 		0
				rect			0 0 224 112
				origin			ORIGIN_WEAPONIMAGE
				style			WINDOW_STYLE_SHADER
				background		"weapon_m1garan1"
				decoration
