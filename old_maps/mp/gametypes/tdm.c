...
Callback_PlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime)
{
	...
	if(isDefined(sWeapon) && sWeapon == "fire_mp"||sWeapon == "axisfire_mp"||sWeapon == "payload_mp")
	{
		...
		if(sWeapon == "payload_mp" && sMeansOfDeath != "MOD_MELEE")
		{
				if (!isDefined(vPoint))
				vPoint = self.origin + (0,0,11);
				if(isDefined(level.cocktail) && level.cocktail == vPoint)
				return;
				level.cocktail = vPoint;
				level thread _mc2\_mc2_mcFX::NapalmMonitorCocktail(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, level.cocktail, vDir, sHitLoc, psOffsetTime);
				return;
		}
