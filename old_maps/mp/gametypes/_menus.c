...
onMenuResponse()
{
	...
	for(;;)
	{
		...
		else if(menu == game["menu_weapon_allies"] || menu == game["menu_weapon_axis"])
		{
			self closeMenu();
			self closeInGameMenu();
			if(_mc2\_mc2_classes::CheckClassAvailablility(response) == "unavailable")
			{
		...
		else if(!level.xenon)
		{
			...
			else if(menu == game["menu_class_special"])
				maps\mp\gametypes\_quickmessages::quickspecials(response);
			else if(response == "class_special")
				self _mc2\_mc2_special::special_effect();
			else if(response == "medic_notify")
				self _mc2\_mc2_special::call_medic();
			else if(response == "ammo_notify")
				self _mc2\_mc2_special::call_support();
